...
# auther: caitongbo
# project: https://github.com/caitongbo/GraduationProject
...
class getLinks():
    ...
    def run(self):
        ...
        while self.depth <= 10: # 每一次深度的爬取都会爬取url_list的所有链接
            print("正在进行第%d轮"%self.depth)
            url_list_tmp = set([])
            for url in url_list:
                print("%d. "%n+url)
                url_list_tmp.update(self.page_getLinks(url))
                n=n+1
...
if __name__ == '__main__':
    ...
    # 'https://www.caitongbo.com/admin?id=6'==>'www.caitongbo.com'
    key = key_tmp.subdomain + '.' + key_tmp.domain+'.' + key_tmp.suffix 
    print('key:', key)
    print('开始爬取...\n')
    getLinks = getLinks(domain = domain, key = key, depth = 1)
    getLinks.run()
